...
class Model(nn.Module):
    def __init__(self, args, data):
        ...
        for idx_class in range(self.num_class):
            self.linears.append( (nn.Linear(self.m, self.m, bias = True))); #w->hid
            self.sparse_label.append(0); self.orthgonal_label.append(1);
            

        ## rolling ##1          
            self.linears.append((nn.Linear(self.RUC_layers*self.hidden_dim, self.reduce_dim, bias = True))); #w->hid
            self.sparse_label.append(0); self.orthgonal_label.append(1);
    ...
    def forward(self, inputs):
        ...
        for window_i in range(self.w):
            ...
            for idx_class in range(self.num_class):
                ...
                if idx_class == 0:
                    x_p_idx = self.linears[self.RUC_layers + idx_class*3+0](x_p);  ## nxhxm (causal)   
                ...
                x_p_idx = self.linears[self.RUC_layers + idx_class*3+1](x_p_idx); ## nxmxr (causal)   
                x_p_idx = torch.sigmoid(x_p_idx/1.);
                x_p_idx = self.dropout(x_p_idx);  
                final_y_tmp_1 = self.linears[self.RUC_layers + idx_class*3+2](x_p_idx).squeeze()
    ...
    def predict_relationship(self):
        ...
        G_0A = self.linears[self.RUC_layers + 0].weight.transpose(0,1).detach().numpy()   
        G_0 = np.abs(G_0A)
        G_original = [] 
        for idx_class in range(self.num_class):       
            A = self.linears[self.RUC_layers + idx_class*3+0].weight.transpose(0,1).detach().numpy() 
            G_tmp = np.abs(A)
            G_tmp[range(self.m), range(self.m)] = 0 
            if idx_class==0:   
                G_tmp = np.abs(G_tmp)
            else:
                G_original.append(G_tmp)
